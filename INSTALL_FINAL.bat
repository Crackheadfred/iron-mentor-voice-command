...
echo Installation de NumPy (etape critique)...
echo Tentative 1: NumPy standard...
pip install numpy
if errorlevel 1 (
    echo ECHEC - Tentative 2: NumPy avec contrainte...
    pip install "numpy>=1.21.0,<2.0.0"
    if errorlevel 1 (
        echo ECHEC - Tentative 3: NumPy version fixe...
        pip install numpy==1.24.3
        if errorlevel 1 (
            echo ATTENTION: NumPy a echoue - on continue sans
        )
    )
)
echo NumPy traite
...
if errorlevel 1 (
    echo ATTENTION: Pygame optionnel - echec
) else (
    echo Pygame installe
)

echo PyAudio...
pip install pyaudio >nul 2>&1
if errorlevel 1 (
    echo ATTENTION: PyAudio optionnel - echec
) else (
    echo PyAudio installe
)

echo PyTorch...
pip install torch >nul 2>&1
if errorlevel 1 (
    echo ATTENTION: PyTorch optionnel - echec
) else (
    echo PyTorch installe
)

echo SciPy...
pip install scipy >nul 2>&1
if errorlevel 1 (
    echo ATTENTION: SciPy optionnel - echec
) else (
    echo SciPy installe
)
...
if success >= 8:
    ...
    print('J.A.R.V.I.S. peut etre lance!')
else:
...
echo.
echo INSTALLATION COMPLETEE - Appuyez sur une touche pour fermer
pause